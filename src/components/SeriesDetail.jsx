...
const SeriesDetail = () => {
  ...
  useEffect(() => {
    ...
    // Si el ID es de Avatar (típicamente 19995 o 76600) o no está en myLibrary como serie, probamos modo película
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#0f171e', color: 'white' }}>
      ...
      <div style={{ 
        ...
        <div style={{ padding: '0 4%', position: 'absolute', bottom: '40px', maxWidth: '850px' }}>
          <h1 style={{ fontSize: '3.5rem', fontWeight: 900, marginBottom: '10px' }}>{tmdb.name || tmdb.title}</h1>
            ...
            <div style={{ display: 'flex', gap: '12px', overflowX: 'auto', paddingBottom: '10px' }}>
              {serie?.temporadasDisponibles.map(t => (
                <button key={t} onClick={() => setTemporadaSel(t.toString())} style={{
                  ...
                  color: 'white', border: 'none', padding: '12px 24px', borderRadius: '30px', cursor: 'pointer', fontWeight: 'bold'
                }}>Temporada {t}</button>
              ))}
            </div>
          )}
      ...
      {/* Grid de Episodios (Solo para series) */}
      {!isMovie && (
        <div style={{ padding: '50px 4%' }}>
          <h2 style={{ fontSize: '1.6rem', marginBottom: '30px', fontWeight: 700 }}>Episodios</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(calc(25% - 20px), 1fr))', gap: '30px 20px' }}>
                ...
                <a key={ep.id} href={localCap?.url || '#'} target="_blank" rel="noreferrer" style={{ textDecoration: 'none', color: 'inherit' }}>
                  ...
                  <div style={{ marginTop: '15px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <h3 style={{ fontSize: '1rem', fontWeight: 700 }}>{ep.episode_number}. {ep.name}</h3>
                      <span style={{ fontSize: '0.8rem', color: '#8197a4' }}>{ep.runtime} min</span>
                    </div>
                    <p style={{ fontSize: '0.85rem', color: '#8197a4', display: '-webkit-box', WebkitLineClamp: '3', WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{ep.overview}</p>
                  </div>
                </a>
              );
            })}
      ...
      <style>{` .ep-card:hover .play-overlay { opacity: 1 !important; } `}</style>
    </div>
  );
};
